...
| `--browser --html FILE` | Render both files over the HTML document `FILE` in a headless Chromium and report where the renders differ, instead of comparing the CSS. The document's own `<style>` elements and stylesheet `<link>`s are removed and its inline `style` attributes kept; the page is loaded afresh under each file at every viewport width a media condition in either file names and under every interaction state either file names, a state applied to every element at once, animations held at their start and transitions finished, and the whole page is captured and compared pixel for pixel. The verdict is the raster and nothing normalises it: two spellings the browser paints alike are one render, whatever `getComputedStyle` says of them. Where a render differs, the computed styles of the elements under the differing pixels are read under both files and what they disagree on is listed, so the report names a property. It records the browser version, viewports, states and how many renders were taken, and `--json` writes the same as a document. Needs node and a headless Chromium (`NODE`, `CHROME`, or the usual places) and exits 2 without them, on a driver failure, or when nothing was rendered. |
...
runs the page and reads matching off the live DOM instead, so it answers for a
class that appears only after a click. Check what your scripts add before
...
| [Custom Properties Level 1](https://www.w3.org/TR/css-variables-1/) | `var()` parsing/printing, typed fallbacks, theme/default substitution |
| [Properties and Values API Level 1](https://www.w3.org/TR/css-properties-values-api-1/) | `@property` registration with `syntax`, `inherits` and `initial-value` |
