...
through them unseen rather than failing to compile. They share one traversal now, written
without that wildcard. A statement kind added after this release stops every
site that has to decide about it from compiling, and a test pins that the
shared walk reaches all twenty-three statement shapes and every place a
declaration can sit. Correctness was judged against a
browser rather than against cascade. The test suite renders a sheet and its
optimised form in headless Chrome, then compares every property
`getComputedStyle` reports on every element; a manual sweep does the same to
live pages. Several of the fixes below are miscompiles Chrome contradicted,
not readings of the spec. Behind those sit 504 CSS files
