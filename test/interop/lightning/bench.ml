...
let () =
  ...
  with Failure msg -> Printf.eprintf "memtrace unavailable: %s\n%!" msg
...
let () =
  ...
  List.iter
    (fun (i, (input, expected)) ->
      ...
      if trace_phase then Printf.eprintf "pair_%04d parse\n%!" i;
      (match Css.of_string input with
      | Error _ -> incr parse_err
      | Ok css -> (
          if trace_phase then Printf.eprintf "pair_%04d print\n%!" i;
          match
      ...
      if dt > 0.05 then
        Printf.eprintf "slow pair_%04d: %.2fs (input %d bytes): %s\n%!" i dt
          (String.length input)
    ...
    pairs;
  Printf.printf "pass=%d fail=%d parse_err=%d total=%d (%.1fs)\n" !pass !fail
    !parse_err total
    (Unix.gettimeofday () -. t_global)
