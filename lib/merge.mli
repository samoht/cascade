...
val identical_global :
  ...
  Stylesheet.rule list ->
  Stylesheet.rule list
(** [identical_global ~same rules] is the body-keyed global analogue of
    {!identical}: it buckets every eligible rule by its body, then greedily
    absorbs every later occurrence into the earliest as long as the gap is
    cascade-safe against the actual candidate's selector (not the head's). Each
    absorption is sound iff every intermediate rule outside the merge group that
    writes one of the body's properties has a selector that does not overlap the
    union of already-accepted member selectors and the candidate's selector. *)
