...
// every viewport a media condition in any sampling sheet turns on, plus the
// default one, and once per interaction state a sampling sheet names. The
// sampling sheets are the rendered ones less what the page cannot use, and
// the rendered sheets themselves when the job has none. A state is written
...
// CSS Images 3 sec. 3.5.3 puts an unpositioned first colour stop at 0% and an
// unpositioned last one at 100%, and the browser serialises the stop as it was
// written. A linear or radial gradient's end stops get those positions, once
// its colours are pixels, so a value that leaves them out is compared as the
// image it paints. A conic gradient's defaults are angles, and it is left as
// it is.
var rdGradientRe = /((?:repeating-)?(?:linear|radial)-gradient)\\(((?:[^()]|\\((?:[^()]|\\([^()]*\\))*\\))*)\\)/g;
function rdFillStops(v) {
  return v.replace(rdGradientRe, function (m, fn, body) {
    ...
    args.push(cur.trim());
    var stops = [];
    for (var k = 0; k < args.length; k++) if (args[k].indexOf('C(') >= 0) stops.push(k);
    if (stops.length < 2) return m;
    var bare = function (arg) { return arg.replace(/C\\([^)]*\\)/, '').trim() === ''; };
    var first = stops[0], last = stops[stops.length - 1];
    if (bare(args[first])) args[first] += ' 0%';
    if (bare(args[last])) args[last] += ' 100%';
    return fn + '(' + args.join(', ') + ')';
  });
}
function rdPaintsSame(a, b) {
  if (a === b) return true;
  var x = rdFillStops(a.replace(rdColourRe, rdPixel));
  var y = rdFillStops(b.replace(rdColourRe, rdPixel));
  if (x === y) return true;
...
function rdMain(job) {
  ...
  var states = [], px = {};
  (job.sampling || job.sheets).forEach(function (sheet) {
